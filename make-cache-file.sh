...
docker run -v $PROJECT_TO_BUILD:/workspace -w /workspace maven:3.5.0-jdk-8 mvn --batch-mode dependency:go-offline >> out.log

# parse download URLs from log and fetch them from maven central
grep -e "Downloaded\:.*\.jar" out.log | awk '{print $3}' | xargs wget -r -nH --cut-dirs=1 $line
